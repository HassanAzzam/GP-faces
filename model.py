...
class GAN:
	...
	def text_encoder(self, t_real_caption):
		gru = tf.nn.rnn_cell.GRUCell(self.options['caption_vector_length'], name='gru_encoder')
		#h_in = tf.placeholder(tf.float32, [1, self.options['caption_vector_length']])
		h_in = gru.zero_state(self.options['batch_size'], dtype='float32')

		return tf.nn.dynamic_rnn(gru, t_real_caption, initial_state=h_in)
	...
	def build_model(self):
		...
		t_real_caption = tf.placeholder('float32', [self.options['batch_size'], self.options['sentence_length'], self.options['word_embedding_vector_length']], name = 'real_caption_input')
		t_z = tf.placeholder('float32', [self.options['batch_size'], self.options['z_dim']])
		
		encoder, encoded_caption = self.text_encoder(t_real_caption)
		# print encoded_caption
		# print f_encoded_state
		# exit()

		fake_image = self.generator(t_z, encoded_caption)
		
		disc_real_image, disc_real_image_logits   = self.discriminator(t_real_image, encoded_caption)
		disc_wrong_image, disc_wrong_image_logits   = self.discriminator(t_wrong_image, encoded_caption, reuse = True)
		disc_fake_image, disc_fake_image_logits   = self.discriminator(fake_image, encoded_caption, reuse = True)
	...
	def sampler(self, t_z, t_text_embedding):
		...
		encoder, encoded_caption = self.text_encoder(t_text_embedding)
		
		s = self.options['image_size']
		s2, s4, s8, s16 = int(s/2), int(s/4), int(s/8), int(s/16)
		
		reduced_text_embedding = ops.lrelu( ops.linear(encoded_caption, self.options['t_dim'], 'g_embedding') )
